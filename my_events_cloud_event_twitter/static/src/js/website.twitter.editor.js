odoo.define('mec_twitter.editor', function (require) {
'use strict';
...
options.registry.twitter = options.registry.marginAndResize.extend({
    ...
    mec_twitter_reload: function(){
        ajax.jsonRpc('/mec_twitter_reload','call', {});
    },
    make_hover_config: function(){
        var self = this;
        var $configuration = $(qweb.render("mec.Twitter.Reload")).hide().appendTo(document.body).click(function (e) {
            e.preventDefault();
            e.stopPropagation();
            self.mec_twitter_reload();
        });
