...
class Twitter(http.Controller):
    @http.route(['/mec_twitter_reload'], type='json', auth="user", website=True)
    def mec_twitter_reload(self):
        return request.website.fetch_favorite_tweets()

    @http.route(['/mec_get_favorites'], type='json', auth="public", website=True)
    def mec_get_tweets(self, limit=20):
        ...
        if not screen_name:
            ...
            return []
        TwitterTweets = request.env['mec.twitter.tweet']
