...
class ResUsers(models.Model):
	...
	def _get_allowed_warehouses(self):
		...
		return [(6, 0, self.env["stock.warehouse"].sudo().search([(1, "=", 1)])._ids)]
	...
	def _get_allowed_locations(self):
		...
		return [(6, 0, self.env["stock.location"].sudo().search([(1, "=", 1)])._ids)]
	...
	def _get_allowed_picking_types(self):
		...
		return [(6, 0, self.env["stock.picking.type"].sudo().search([(1, "=", 1)])._ids)]
