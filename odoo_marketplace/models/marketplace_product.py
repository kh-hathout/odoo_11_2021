    ...
    def check_state_send_mail(self):
        ...
        for obj in self.filtered(lambda o: o.status in ["approved", "rejected"]):
            ...
            if obj.status == "approved":
                temp_id = config_setting_obj["notify_admin_on_product_approve_reject"]
                temp_id2 = config_setting_obj["notify_seller_on_product_approve_reject"]
            elif obj.status == "rejected":
                temp_id = config_setting_obj["notify_admin_on_product_approve_reject"]
                temp_id2 = config_setting_obj["notify_seller_on_product_approve_reject"]
            # Notify to admin by admin when product approved/rejct
            if config_setting_obj["enable_notify_admin_on_product_approve_reject"] and config_setting_obj.get("notify_admin_on_product_approve_reject", False):
                if temp_id:
