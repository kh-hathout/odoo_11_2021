...
class SaleOrder(models.Model):
	...
	def action_confirm(self):
		...
		if config_setting_obj.get("enable_notify_seller_on_new_order", False) and config_setting_obj.get("notify_seller_on_new_order", False):
			template_obj = self.env['mail.template'].browse(config_setting_obj.get("notify_seller_on_new_order"))
			for order in self:
...
class SaleOrderLine(models.Model):
	...
	def _compute_create_year(self):
		for sol in self:
			sol.create_year = dateutil.parser.parse(sol.create_date).date().year
