...
class ResPartner(models.Model):
    ...
    status_msg = fields.Text(string="Account Status Message", translate=True, default=lambda self: self.env["res.config.settings"].mp_config_translatable().get("seller_new_status_msg", "Thank you, for registering with us we have already received your request but to enjoy the benefits of our Marketplace and to get your request approved quickly, fill your details below."))
    ...
    def _compute_sol_count(self):
        """ """
        for obj in self:
            obj.sol_count = len(self.env["sale.order.line"].search([('state','in',['sale','done']),("marketplace_seller_id", "=", obj.id)]))
    ...
    def on_change_payment_assest(self):
        if self.commission < 0 or self.commission >= 100:
            raise Warning(_('Commission should be in "0 to 100" Range.'))
    ...
    def on_change_seller_wise_settings(self):
        if not self.set_seller_wise_settings:
            ...
            vals["show_seller_since"] = self.env['ir.default'].get(
                'res.config.settings', 'seller_since')
            vals["show_seller_address"] = self.env['ir.default'].get(
                'res.config.settings', 'shipping_address')
            vals["show_product_count"] = self.env['ir.default'].get(
                'res.config.settings', 'product_count')
            vals["show_sale_count"] = self.env['ir.default'].get(
                'res.config.settings', 'sale_count')
            vals["show_return_policy"] = self.env['ir.default'].get(
                'res.config.settings', 'return_policy')
            vals["show_shipping_policy"] = self.env['ir.default'].get(
                'res.config.settings', 'shipping_policy')
            vals["show_seller_review"] = self.env['ir.default'].get(
                'res.config.settings', 'seller_review')
            self.update(vals)
    ...
    def on_change_seller(self):
        ...
        self.warehouse_id = config_setting_obj[
            "default_warehouse"] if config_setting_obj.get("default_warehouse") else False
    ...
    def write(self, vals):
        ...
        for rec in self:
            if rec.seller:
                # Update mail notification to seller and admin on denied -> pending state change issue
                if rec.state != vals.get("state", ""):
                    if rec.state in ["new","denied"] and vals.get("state", "") == "pending":
                        pending_seller = True
    ...
    def get_seller_global_settings(self, config_setting_obj):
        self.ensure_one()
        warehouse_id = config_setting_obj.get("default_warehouse")
        ...
        default_warehouse = self.env["stock.warehouse"].browse(warehouse_id)
        warehouse_location_id = self.env["stock.location"].browse(location_id)
        global_settings = {
            "commission": config_setting_obj.get("global_commission"),
            "warehouse_id": default_warehouse.name_get()[0] if default_warehouse else None,
            ...
            "show_seller_since": config_setting_obj.get("seller_since"),
            "show_seller_address": config_setting_obj.get("shipping_address"),
            "show_product_count": config_setting_obj.get("product_count"),
            "show_sale_count": config_setting_obj.get("sale_count"),
            "show_return_policy": config_setting_obj.get("return_policy"),
            "show_shipping_policy": config_setting_obj.get("shipping_policy"),
            "show_seller_review": config_setting_obj.get("seller_review"),
        }
    ...
    def get_limit(self):
        config_setting_obj = self.env[
            'res.config.settings'].sudo().get_values()
        if config_setting_obj.get('review_load_no', False):
    ...
    def total_active_recommendation(self):
        ...
        return_list.append(round(yes_percentage,2))
        return return_list
