...
class ResUsers(models.Model):
    ...
    def signup(self, values, token=None):
        ...
        elif values.get('url_handler', False):
            values.pop("url_handler")
    ...
    def copy(self, default=None):
        ...
        if self._context.get('is_seller', False):
            ...
            wk_valse = {
                ...
                "warehouse_id": self.env['ir.default'].get('res.config.settings', 'default_warehouse') or False,
    ...
    def notification_on_partner_as_a_seller(self):
        ...
        for user_obj in self:
            if user_obj.partner_id.seller:
                ...
                if config_setting_obj["enable_notify_admin_4_new_seller"] and config_setting_obj.get("notify_admin_4_new_seller",False) and config_setting_obj["notify_admin_4_new_seller"]:
                    # Notify to admin by admin on new seller creation
                    temp_id = config_setting_obj["notify_admin_4_new_seller"]
                    if temp_id:
                        template.browse(temp_id).send_mail(user_obj.partner_id.id, True)
                if config_setting_obj["enable_notify_seller_4_new_seller"] and config_setting_obj.get("notify_seller_4_new_seller",False) and config_setting_obj["notify_seller_4_new_seller"]:
                    # Notify to Seller by admin on new seller creation
                    temp_id2 = config_setting_obj["notify_seller_4_new_seller"]
                    if temp_id2:
